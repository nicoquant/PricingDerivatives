...
class ARMA():
    def __init__(self, serie, order):
        ...
        if len(order) == 2:
            ...
            print('Can you please enter a value for p and q ? No need to enter a value for integrating the process,'
    ...
    def AR_YW_estimate(self):
        ...
        return inv(R_p).dot(r_p[1:])  # r_p[1:]: since we do not take corr(Xt, Xt) in account
    ...
    def AR_least_squared_estimator(self):
        ...
        X = np.array([shift(self.serie, i, cval=np.NaN) for i in range(1, self.lag + 1)]).T[self.lag:]
    ...
    def ar_estimation(self, initial_guess = None):
        ...
        if initial_guess == None:
            ...
            try:
                ...
                print("Please fix your starting parameters. The optimization has been done using a set of 0 values as "
                      "starting parameters.")
                initial_guess = np.array([0] * self.ar_param)
    ...
    def stationarity_check(self, estimation_method='ls'):
        ...
        if estimation_method == 'ls':
            ...
            method = input("Please choose an estimation method between: 'ls' and 'yw' or 'exit' to exit: ")
    ...
    def stationary_test_adf(self):
        ...
        if adfuller(self.serie)[1] > 0.05:
            print('Warning: Your stability condition is not satisfied. Please, make your serie stationary')
        else:
            print('Stationarity check: Pass')
    ...
    def integrated_process(self, estimation_method):
        ...
        if estimation_method == 'ls':
            ...
            estimation_method = input("Please choose an estimation method between: 'ls' and 'yw' or 'exit' to exit: ")
            if estimation_method == 'exit':
                sys.exit()
    ...
    def loglikelihood_ma(self, param):
        ...
        for t in range(1, len(self.serie)):
            eta2.insert(0, self.serie[t] - np.mean(self.serie) - param @ np.array(eta2[:len(param)]))


        return T / 2 * np.log(np.var(self.serie)) + 1 / (2 * np.var(self.serie)) * np.sum(
            np.array(eta2) ** 2) + np.mean(self.serie)
    ...
    def loglikelihood_arma(self, param):
        ...
        for t in range(len(param_ar), T):
            eta2.insert(0, param_ar @ np.flip(self.serie[t - len(param_ar): t]) - mu - param_ma @ np.array(eta2[:len(param_ma)]))

        return T / 2 * (np.log10(np.var(self.serie))) +  np.log10(2*np.pi) + np.sum(
            (np.array(eta2) ** 2) / (2*np.var(self.serie)))
...
if __name__ == '__main__':
    ...
    print("")
    print("")
    print('Loglikelihood obtained by this minimization: ' + str(model.loglike([np.mean(serie)] + list(arma_est) + [np.var(serie)])))
    print('Parameters obtained by this estimation: ' + str(arma_est))

    print("")
    print("")

    print('Loglikelihood obtained by statsmodel minimization: ' + str(model.loglike(model_fit.params)))
    print('Parameters obtained by statsmodel estimation: ' + str(model_fit.params[1:-1]))
